...
function (eset_full, datatypes, permute_labels, parallel = FALSE, 
          ncores = 4) 
{
  ...
  if (parallel == FALSE) {
    null_interact <- lapply(1:n_permute, function(each_null) {
      emat1 <- exprs(eset_sub)[permute_labels[[each_null]][, 
                                                           order_datatypes[1]], eset_sub$seqData == datatypes[1]]
      emat2 <- exprs(eset_sub)[permute_labels[[each_null]][, 
                                                           order_datatypes[2]], eset_sub$seqData == datatypes[2]]
